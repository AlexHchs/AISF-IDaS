...
def load_testbed_information(UE_ID):
    # Load testbed's information from database
    UE_ID = UE_ID + 1
    info_controller = Controller.objects.get(UE_id = UE_ID)
    info_dls = DataLoggerServer.objects.get(UE_id = UE_ID)
    info_ts = TargetServer.objects.get(UE_id = UE_ID)
    info_bs = BenignServer.objects.get(UE_id = UE_ID)
    info_list_vc = VulnerableClient.objects.get(UE_id = UE_ID)
    info_list_nvc = NonVulnerableClient.objects.all(UE_id = UE_ID)
    info_as = AttackerServer.objects.get(UE_id = UE_ID)
    info_mc = MaliciousClient.objects.get(UE_id = UE_ID)

    # Setting attack scenario information from database
    info_attack_scenario = AttackScenario.objects.get(UE_id = UE_ID)
    ...
    # Setting needed machine learning models from database
    models_name = []
    info_machine_learning_model = MachineLearningModel.objects.get(UE_id = UE_ID)
    ...
    # Setting skip stages from database
    info_skip_stage = SkipStage.objects.get(UE_id = UE_ID)
